...
- [x] Point release-mode resolution at the packed `python/python.exe`; delete
...
manifest verifier passed against an ignored assembled CPU runtime after removing
bytecode generated by an earlier probe that did not use `-B`; the final release
spawn now places `-B` before `main.py`, and the embedded interpreter reported
isolated mode, no user-site, and shipped Torch/FastAPI imports. The real
path-with-spaces/non-ASCII authenticated launch smoke remains part of the Stage
7 clean-runner gate.
